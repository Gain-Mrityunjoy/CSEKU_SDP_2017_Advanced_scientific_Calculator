...
public class HexMode {
    ...
    public String[] processString(String sMath){
        ...
        for (int i=0; i<sMath.length(); i++){
            ...
            if (i<sMath.length()-1 && isCharPi(c) && !HM.isOperator(sMath.charAt(i+1))){
                check_error = true;
                return null;
            }
            else
            ...
            if (!HM.isOperator(c))
                ...
                if (c == '(') S.push(elementMath[i]);
                    ...
                    if (c == ')'){
                        ...
                        while (!S.isEmpty() && HM.priority(S.peek().charAt(0)) >= HM.priority(c))
                            s1 = s1 + S.pop() + " ";
    ...
    public String decToDec(int num){
        String b;
    ...
    public String valueMath(String[] elementMath){
        ...
        for (int i=0; i<elementMath.length; i++){
            ...
            if (!HM.isOperator(c)) S.push(Integer.parseInt(elementMath[i]));
            else{
